...
/*   Updated: 2024/03/05 11:17:58 by vcaratti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	rem[OPEN_MAX][BUFFER_SIZE];
	...
	if (BUFFER_SIZE <= 0 || fd < 0 || fd > OPEN_MAX)
		return (NULL);
	if (has_endl(rem[fd], 0) >= 0)
		return (get_first_line(rem[fd]));
	mem = array_to_mem(NULL, rem[fd]);
	if (mem == NULL)
		return (NULL);
	while (has_endl(mem, 1) < 0)
	{
		...
		if (b_read <= 0)
			return (empty_mem_rem(&mem, rem[fd], b_read));
		mem = array_to_mem(mem, buffer);
		if (mem == NULL)
			return (NULL);
	}
	return (mem_to_array_ret_firstl(mem, rem[fd]));
}
